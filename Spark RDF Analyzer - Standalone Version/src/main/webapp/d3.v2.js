(function() {
  ...
  d3.random = {
    normal: function(mean, sigma) {
      ...
      return function() {
        ...
        return mean + sigma * x * Math.sqrt(-2 * Math.log(r) / r);
      };
    },
    logNormal: function(mean, sigma) {
      var n = arguments.length;
      if (n < 2) sigma = 1;
      if (n < 1) mean = 0;
      var random = d3.random.normal();
      return function() {
        return Math.exp(mean + sigma * random());
      };
    },
  ...
  var d3_formatPrefixes = [ "y", "z", "a", "f", "p", "n", "mean", "m", "", "k", "M", "G", "T", "P", "E", "Z", "Y" ].map(d3_formatPrefix);
