...
/*
 * This class calculates Chord Data and formats them for required output Chord or Table.
 */
public class EdgeFinder {
	 ...
	 */
   DataFrame resultsFrame = WebService.sqlContext.sql("SELECT t2o AS s, t1p AS p, t4o AS o, Count(*) AS nr FROM (SELECT t1.subject AS t1s, t2.object AS t2o, t1.predicate AS t1p, t1.object AS t1o FROM Graph t1, Graph t2"
	...
	if(args[1].equals("Table"))
	   {
		   ...
		   {
			   result +="<tr><td data-toggle=\"tooltip\" title=\""+r.getString(0)+"\">"+Configuration.shortenURI(r.getString(0))+"</td><td data-toggle=\"tooltip\" title=\""+r.getString(1)+"\">"+Configuration.shortenURI(r.getString(1))+"</td><td data-toggle=\"tooltip\" title=\""+r.getString(2)+"\">"+Configuration.shortenURI(r.getString(2))+"</td><td>"+Long.toString(r.getLong(3))+"</td></tr>";
	   ...
	   else if(args[1].equals("Chart"))
	   {
			...
			for(Row r : resultRows)
			{
				matrix[ArrayUtils.indexOf(classNames, Configuration.shortenURI(r.getString(0)))][ArrayUtils.indexOf(classNames, Configuration.shortenURI(r.getString(2)))] += Integer.parseInt(Long.toString(r.getLong(3))); 
			}
	...
	public static String partialRead(String inputSubject)
	{
		...
		result = "<table class=\"table table-striped\">";
		   ...
		   for(Row r : cachedRows)
		   {
			   ...
			   {
				   result +="<tr><td data-toggle=\"tooltip\" title=\""+r.getString(0)+"\">"+Subject+"</td><td data-toggle=\"tooltip\" title=\""+r.getString(1)+"\">"+Configuration.shortenURI(r.getString(1))+"</td><td data-toggle=\"tooltip\" title=\""+r.getString(2)+"\">"+Object+"</td><td>"+Long.toString(r.getLong(3))+"</td></tr>";
			   }
		   }
		   result +="</table>";
